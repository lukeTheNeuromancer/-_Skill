# Three.js 入门教程：从 0 创建第一个 3D Web 应用（修订增强版）

> Three.js 是一个基于 JavaScript 的 Web 3D 引擎，封装了 WebGL，让你在浏览器里就能做出 3D 场景、动画、游戏、数据可视化，甚至 AI + 3D 应用。

**这份教程改了什么？**
原稿的知识结构是对的，但缺了一个关键环节：`npm run dev` 之前从没配置过开发服务器，照着抄是跑不起来的。这版补上了 Vite 环境搭建、窗口自适应、控制器阻尼这些"能跑起来"必需的细节，并在每个阶段加了「检查点」和「小挑战」，让你确认自己没有掉队，也让教程更好玩。

文末还附了一个**可以直接打开运行**的成品 Demo（`threejs-demo.html`），拖一拖、点一点，先感受一下终点长什么样，再回来跟着做。
...
我没有直接在教程里塞静态截图，原因很实际：**这份教程的每个阶段都是动态、可交互的结果**（旋转、拖拽视角、光影变化），一张静态图能传达的信息远不如你自己跑起来看一眼。所以我做了一个更有用的替代品：

📎 **`threejs-demo.html`** —— 教程最终效果的可运行版本，双击直接在浏览器打开就能玩（拖拽旋转、滚轮缩放、点击立方体换色），不需要装 Node、不需要跑 `npm install`。建议你**先打开玩一玩，再跟着教程做**，心里有个目标画面，学起来会更有方向感。
